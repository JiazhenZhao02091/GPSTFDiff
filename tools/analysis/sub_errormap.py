"""
功能:
    从误差图目录的各模型子文件夹中裁剪指定区域，并保存带红框的全图参照。

参数:
    --input_dir (str): 包含多个模型子文件夹的输入目录。
    --output_dir (str): 裁剪结果输出目录。
    CROP_BBOXES (list[list[int]]): 脚本内配置的裁剪框列表，格式为 [y_start, y_end, x_start, x_end]。

返回:
    None。函数直接保存图片文件。

输出:
    输出带模型名前缀的裁剪 PNG 和 full_with_boxes PNG 文件。
"""
import os
import argparse
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_dir', type=str, 
                        default='/home/zhaojiazhen/workspace/STF/STF/artifacts/image/error_map/ML/individual', 
                        help='包含多个模型子文件夹的基础目录')
    parser.add_argument('--output_dir', type=str, 
                        default='/home/zhaojiazhen/workspace/STF/STF/artifacts/image/error_map/ML/individual_sub_boxes', 
                        help='保存裁剪后图像的目标目录')
