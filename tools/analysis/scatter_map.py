"""
功能:
    为 CIA、LGC、ML 数据集生成 RED/NIR 预测值与真值的散点图、密度散点图和色标。

参数:
    --dataset (str): 数据集名称，通常为 CIA、LGC 或 ML。
    true_dir (str): main 中按数据集配置的真实影像目录。
    pred_dirs_dict (dict[str, str]): 模型名称到预测影像目录的映射。
    out_dir (str): 绘图结果输出目录。

返回:
    None。main 函数不返回数据对象。

输出:
    在 artifacts/image/scatter_map/<dataset> 下保存组合图、单模型图和 colorbar PNG 文件。
"""
import os
import glob
