"""
功能:
    为 CIA、LGC、ML 全图预测结果生成绝对误差图、模型对比网格图和独立色标。

参数:
    --dataset (str): 数据集名称，可选 CIA、LGC、ML。
    true_dir (str): 真实影像目录。
    pred_dirs_dict (dict[str, str]): 模型名称到预测影像目录的映射。
    output_dir (str): 误差图输出目录。
    enable_grid_plot (bool): 是否额外保存模型对比网格图。
    cmap (str): matplotlib 色带名称。

返回:
    None。函数直接保存图片文件。

输出:
    在 artifacts/image/error_map/<dataset> 下保存 individual 误差图、Comparison 图和 colorbar PNG 文件。
"""
import os
import numpy as np
import tifffile
...
if __name__ == '__main__':
    ...
    if args.dataset == 'CIA':
        ...
        OUTPUT_DIR = '/home/zhaojiazhen/workspace/STF/STF/artifacts/image/error_map/CIA'
        PRED_DIRS_DICT = {
        ...
        OUTPUT_DIR = '/home/zhaojiazhen/workspace/STF/STF/artifacts/image/error_map/LGC'
        PRED_DIRS_DICT = {
        ...
        OUTPUT_DIR = '/home/zhaojiazhen/workspace/STF/STF/artifacts/image/error_map/ML'
        PRED_DIRS_DICT = {
