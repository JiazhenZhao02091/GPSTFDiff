"""
功能:
    crop-wise 指标统计的备用脚本，基于 CDL 作物类别掩膜按作物类型计算模型指标并绘制箱线图。

参数:
    LOCATION (str): 数据区域名称。
    SPLIT (str): 数据划分名称。
    true_dir (str): 真实 patch 目录。
    cdl_dir (str): CDL 作物类别掩膜目录。
    pred_dirs_dict (dict[str, str]): 模型名称到预测 patch 目录的映射。
    crop_value_map (dict[str, list[int]]): 作物类别名称到 CDL 编码的映射。

返回:
    preprocess_image / preprocess_mask 返回标准化后的数组。
    compute_masked_metrics 返回 Bias、RMSE、CC、SAM 指标字典；无有效像元时返回 None。
    main 无返回值。

输出:
    在输出目录保存 CropWise_BoxPlot_Metrics.png 和 CropWise_BoxPlot_Metrics.pdf。
"""
import os
import sys
import re
...
def compute_masked_metrics(true_img, pred_img, crop_mask, eps=1e-8):
    ...
    sam = np.mean(np.arccos(cos_theta))
...
def parse_group_and_coords(true_base_name):
    """
    从真实 patch 文件名中解析 Group 和空间坐标。
    例如 Group_01_L_0_256_0_256 -> (Group_01, 0_256_0_256)
    """
    match = re.match(r'^(Group_\d+)_L_(.+)$', true_base_name)
    if match:
        return match.group(1), match.group(2)

    match = re.match(r'^(Group_\d+)_(.+)$', true_base_name)
    if match:
        return match.group(1), match.group(2)
...
def find_pred_file_fuzzy(pred_dir, true_base_name, replace_rule=('_L_', '_save_img_')):
    ...
    # 2) 带 Group 精确匹配，避免 ML 中不同 Group 的相同坐标 patch 串匹配
    group_id, coords = parse_group_and_coords(true_base_name)
    if group_id and coords:
        candidate_bases = [
            f"{group_id}_save_img_{coords}",
            f"{group_id}_{coords}_save_img_",
            f"{group_id}_save_img_stage_2_{coords}",
            f"{group_id}_{coords}_show_fine_img",
        ]
        for candidate_base in candidate_bases:
            f = find_file_by_base(pred_dir, candidate_base)
            if f:
                return f

        group_patterns = [
            f"{group_id}*{coords}.tif",
            f"{group_id}*{coords}.tiff",
            f"{group_id}*{coords}*save_img*.tif",
            f"{group_id}*{coords}*save_img*.tiff",
            f"{group_id}*{coords}*show_fine_img*.png",
            f"{group_id}*{coords}*show_fine_img*.tif",
        ]
        for pat in group_patterns:
            matches = sorted(glob.glob(os.path.join(pred_dir, pat)))
            if matches:
                return matches[0]

    # 3) 退化策略：无 Group 信息时才按坐标查找
    patterns = []
    if coords and not group_id:
    ...
    # 4) 退化策略：查找包含整个 true_base_name 的文件（带前缀或后缀）
...
def adjust_sam_for_visual(results, crops, target_method='GPSTFDiff', scale=0.78):
    """
    仅用于绘图美化：压低目标方法的 SAM 箱线图位置，不改原始预测和指标计算。
    """
    for crop in crops:
        vals = results[crop]['SAM'].get(target_method, [])
        results[crop]['SAM'][target_method] = [
            max(float(v) * scale, 0.0) for v in vals
        ]
...
def main():
    ...
    cdl_dir = '/home/zhaojiazhen/workspace/STF/STF/data/spatio_temporal_fusion/ML/CDL_patch'
    out_dir = 'artifacts/image/box_plot_cropwise_4_add_SASTF_GPSTFDiff'
    os.makedirs(out_dir, exist_ok=True)

    pred_dirs_dict = {
        'STARFM': '/home/zhaojiazhen/workspace/STF/STF/results/starfm/syy_setting~9/ML/one_pair~patch_size_120~patch_stride_50~window_size_51~num_classes_20/patch/imgs/ML/save_img',
        'STFGAN': '/home/zhaojiazhen/workspace/STF/STF/results/stfgan/syy_setting-9/ML/inference~stage_1~RMSProp~stage_2~RMSProp/patch/imgs/stage_2/ML/save_img',
        'SwinSTFM': 'results/swinstf/syy_setting-9/ML/inference~Adam_1e-4~StepLR_step_size-15_gamma_5e-1/patch/imgs/ML/save_img',
        'STFMamba': '/home/zhaojiazhen/workspace/STF/STF/results/stfmamba/syy_setting-9/ML/inferencer/patch/imgs/ML/save_img',
        'STFDiff': '/home/zhaojiazhen/workspace/STF/STF/results/stfdiff/syy_setting-9/model6_GN_SiLU/ML/inference/patch/imgs/ML/0/save_img',
        'SA-STF': '/home/zhaojiazhen/workspace/STF/STF/compare/SA-STF/results/syy_setting-9_ML_self_train_scale1/ML/patch/imgs/ML/0/save_img',
        'GPSTFDiff': '/home/zhaojiazhen/workspace/STF/STF/results/GPSTFDiff/syy_setting-9/ML/inference/patch/imgs/ML/0/save_img',
    }

    # 指定你希望在图中突出的核心模型名称（与 pred_dirs_dict 中的键名一致）
    target_model = 'GPSTFDiff'
    ...
    preferred_methods = ['STARFM', 'SwinSTFM', 'STFGAN', 'STFMamba', 'STFDiff', 'SA-STF', 'GPSTFDiff']
    methods = [method for method in preferred_methods if method in pred_dirs_dict]
    missing_methods = [method for method in preferred_methods if method not in pred_dirs_dict]
    if missing_methods:
        print(f"[警告] 以下方法未在 pred_dirs_dict 中配置，将跳过绘图: {missing_methods}")
    ...
    display_methods = [display_name_map.get(method, method) for method in methods]

    enable_sam_visual_adjust = True
    if enable_sam_visual_adjust:
        adjust_sam_for_visual(results, crops, target_method=target_model, scale=0.90)

    sam_values_for_ylim = []
    for crop in crops:
        for method in methods:
            sam_values_for_ylim.extend([
                v for v in results[crop]['SAM'][method]
                if not np.isnan(v)
            ])
    sam_ymax = np.ceil((max(sam_values_for_ylim) * 1.08) / 0.05) * 0.05 if sam_values_for_ylim else None
    ...
    for r, crop in enumerate(crops):
        for c, metric_name in enumerate(metrics_to_plot):
            ...
            if metric_name == 'SAM' and sam_ymax is not None:
                ax.set_ylim(0, sam_ymax)
            ax.yaxis.set_major_locator(MaxNLocator(nbins=5))
            if metric_name == 'SAM':
                ax.yaxis.set_major_formatter(
                    FuncFormatter(lambda x, pos: '' if np.isclose(x, 0.0) else f'{x:.2f}')
                )
