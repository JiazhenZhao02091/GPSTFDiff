...
def offline_evaluation_report(
    ...
    if len(gt_img_path_list) != len(pred_img_path_list):
        raise ValueError(
            f'GT image count ({len(gt_img_path_list)}) != pred image count '
            f'({len(pred_img_path_list)}) for {exp_name}'
        )
    ...
    for i in range(img_num):
        ...
        for metric in metric_list:
            ...
            if isinstance(metric_value, torch.Tensor) and metric_value.numel() > 1:
                avg_val = metric_value.mean().item()
    ...
    for metric in metric_list:
        ...
        if mean_val.size > 1:
            ...
            summary[metric_name] = {
                'global': avg_global,
                'bands': np.asarray(mean_val).tolist(),
            }
        else:
            avg_global = float(mean_val)
            avg_msg += f', {metric_name}: {avg_global:.4f}'
            summary[metric_name] = avg_global
...
if __name__ == "__main__":
    ...
    # pred_dir_path_stf = "results/GPSTFDiff/lap/syy_setting-9/ML/inference/full/imgs/ML/0/save_img"
    ...
    pred_dir_path_stf = "results/GPSTFDiff/syy_setting-9/CIA/ablation/inference_DDPM/patch/imgs/CIA/0/save_img"
