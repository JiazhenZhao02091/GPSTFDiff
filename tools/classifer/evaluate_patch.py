...
try:
    ...
    try:
        ...
        DATASET_SETTING = {}
CLASS_NAMES = {0: 'Corn', 1: 'Soybean', 2: 'Grassland', 3: 'Forest', 4: 'Developed', 255: 'Background'}
...
def plot_confusion_matrix(cm, classes, save_path, title='Confusion Matrix'):
    ...
    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            ax.text(j, i, f'{cm_norm[i, j]:.2f}', ha='center', va='center', color='white' if cm_norm[i, j] > thresh else 'black')
    fig.tight_layout()
...
def get_inference_config(location, split='val', setting_dict=DATASET_SETTING):
    ...
    for idx, triplet in enumerate(triplets):
        ...
        if not target_date:
            continue
        dataset_split.append({'group_id': f'Group_{idx + 1:02d}', 'date': target_date, 'year': get_year_from_filename(target_date) or target_date.split('-')[0]})
...
def evaluate_method_patch(method_name, pred_dir, cdl_dir, dataset_split, model, device, save_dir):
        ...
        for pred_file in pred_patch_files:
            ...
            try:
                ...
                if len(g) > 0:
                    ...
                    miou = np.mean(intersection / (union + 1e-06))
    ...
    if total_patches_evaluated > 0:
        ...
        class_f1 = 2 * precision * recall / np.maximum(precision + recall, 1e-08)
        report = []
        report.append(f"\n{'*' * 50}")
...
if __name__ == '__main__':
    LOCATION = 'ML'
    CDL_DIR = os.path.join(PROJECT_ROOT, 'data/spatio_temporal_fusion/ML/CDL_patch')
    BASE_LOG_DIR = os.path.join(PROJECT_ROOT, 'PPT/crop_unet/all_methods_eval_patch')
    os.makedirs(BASE_LOG_DIR, exist_ok=True)
    METHODS_DICT = {'starfm': os.path.join(PROJECT_ROOT, 'results/starfm/ML/patch'), 'FSDAF': os.path.join(PROJECT_ROOT, 'results/FSDAF/ML/patch'), 'FitFC': os.path.join(PROJECT_ROOT, 'results/FitFC/ML/patch'), 'stfdcnn': os.path.join(PROJECT_ROOT, 'results/stfdcnn/ML/patch'), 'ganstfm': os.path.join(PROJECT_ROOT, 'results/ganstfm/ML/patch'), 'stfgan': os.path.join(PROJECT_ROOT, 'results/stfgan/ML/patch'), 'opgan': os.path.join(PROJECT_ROOT, 'results/opgan/ML/patch'), 'swinstf': os.path.join(PROJECT_ROOT, 'results/swinstf/ML/patch'), 'fsdformer': os.path.join(PROJECT_ROOT, 'results/fsdformer/ML/patch'), 'stfmamba': os.path.join(PROJECT_ROOT, 'results/stfmamba/ML/patch'), 'stfdiff': os.path.join(PROJECT_ROOT, 'results/stfdiff/ML/patch'), 'lapstfdiff': os.path.join(PROJECT_ROOT, 'results/LapSTFDiff/ML/patch'), 'sa-stf': os.path.join(PROJECT_ROOT, 'results/SA_STF/ML/patch')}
    ...
    model = smp.Unet(encoder_name='resnet18', in_channels=CONFIG['in_channels'], classes=CONFIG['num_classes'], encoder_weights=None)
    model.load_state_dict(torch.load(CONFIG['model_path'], map_location=CONFIG['device']))
    model.to(CONFIG['device'])
    dataset_split = get_inference_config(LOCATION, split='test')
    if not dataset_split:
        print('警告：从 syy_setting 获取失败，启用内置 fallback 配置...')
        dataset_split = [{'group_id': 'Group_01', 'year': '2021'}, {'group_id': 'Group_02', 'year': '2022'}, {'group_id': 'Group_03', 'year': '2023'}, {'group_id': 'Group_04', 'year': '2023'}]
    for method_name, pred_dir in METHODS_DICT.items():
        evaluate_method_patch(method_name=method_name, pred_dir=pred_dir, cdl_dir=CDL_DIR, dataset_split=dataset_split, model=model, device=CONFIG['device'], save_dir=BASE_LOG_DIR)
    print('\n所有模型评估已全部完成！')
