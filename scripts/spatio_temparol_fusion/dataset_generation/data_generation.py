...
if __name__ == '__main__':
    ...
    for dataset_type in DATASET_TYPE:
        ...
        for phase in phase_list:
            ...
            valid_patch_stems_per_group = []
            if 'crop' in src_data_prefix_tmpl:
                for data_group in data_group_list:
                    group_valid_stems = None
                    for sensor_type in SENSOR_TYPE:
                        temp_src_data_dir_path = (
                            root_path
                            / dataset_type
                            / src_data_prefix
                            / 'original'
                        ...
                        sensor_group_stems = None
                        for data_date in data_group:
                            _files = list(
                                temp_src_data_dir_path.glob(f'*{data_date}*.tif')
                            ) + list(
                                temp_src_data_dir_path.glob(f'*{data_date}*/*.tif')
                            )
                            _stems = {f.stem.split('_', 1)[-1] for f in _files}

                            if sensor_group_stems is None:
                                sensor_group_stems = _stems
                            else:
                                sensor_group_stems &= _stems
                        if sensor_group_stems is None:
                            sensor_group_stems = set()
                        if group_valid_stems is None:
                            group_valid_stems = sensor_group_stems
                        else:
                            group_valid_stems &= sensor_group_stems
                    if group_valid_stems is None:
                        valid_patch_stems_per_group.append(set())
                    else:
                        valid_patch_stems_per_group.append(group_valid_stems)
                ...
                for data_group_idx, data_group in enumerate(pbar):
                    ...
                    for data_date in data_group:
                        ...
                        for src_data_path in src_data_path_list:
                            ...
                            check_stem = tar_stem.split('_', 1)[-1]
                            if (
                                'crop' in src_data_prefix_tmpl
                                and check_stem
                                not in valid_patch_stems_per_group[data_group_idx]
                            ):
                                continue
