...
img_channel_num = 6
model_generator = STFGANGenerator(img_channel_num=img_channel_num)
model_discriminator = STFGANDiscriminator(img_channel_num=img_channel_num)

optimizer_generator = partial(torch.optim.RMSprop, lr=1e-4)
optimizer_discriminator = partial(torch.optim.RMSprop, lr=1e-4)

scheduler_generator = partial(torch.optim.lr_scheduler.StepLR, step_size=250, gamma=0.1)
scheduler_discriminator = partial(
    torch.optim.lr_scheduler.StepLR, step_size=250, gamma=0.1
)
