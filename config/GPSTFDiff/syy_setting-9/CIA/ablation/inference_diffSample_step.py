...
from src.model.GPSTFDiff.GPSTFDiff_ablation.diffusion_lap_inferencer_diffSample_step import Diffusion
...
"""
    修改 sample timestep 和 T1 T2 即可
    sample T1  = T1 * sample timesteps / num_train_steps 
    sample T2  = T2 * sample timesteps / num_train_steps 
    对于 DDIM 和 DDPM 也是类似方式，不过不用使用 T1 和 T2 去做约束
"""
model = Diffusion(
