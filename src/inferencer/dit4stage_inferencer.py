...
class Inferencer:
    def __init__(
        ...
        #
        model=None,
        checkpoint_path_x4=None,
        checkpoint_path_x3_x4=None,
        checkpoint_path_x2_x3_x4=None,
        checkpoint_path_x1_x2_x3_x4=None,
        #
        metric_list=None,
    ):
        ...
        self.init_model(model, checkpoint_path_x4, checkpoint_path_x3_x4, checkpoint_path_x2_x3_x4, checkpoint_path_x1_x2_x3_x4)
    ...
    def get_model_ema_dict(self, checkpoint_path):
    ...
    def init_model(self, model, checkpoint_path_x4, checkpoint_path_x3_x4, checkpoint_path_x2_x3_x4, checkpoint_path_x1_x2_x3_x4):
        self.device = 'cuda'
        self.model = model.to(self.device)
        self.ema = EMA(self.model, beta=0.995, update_every=1).to(self.device)
        
        model_dict_x4, ema_dict_x4 = self.get_model_ema_dict(checkpoint_path_x4) if checkpoint_path_x4 else (None, None)
        model_dict_x3_x4, ema_dict_x3_x4 = self.get_model_ema_dict(checkpoint_path_x3_x4) if checkpoint_path_x3_x4 else (None, None)
        model_dict_x2_x3_x4, ema_dict_x2_x3_x4 = self.get_model_ema_dict(checkpoint_path_x2_x3_x4) if checkpoint_path_x2_x3_x4 else (None, None)
        model_dict_x1_x2_x3_x4, ema_dict_x1_x2_x3_x4 = self.get_model_ema_dict(checkpoint_path_x1_x2_x3_x4) if checkpoint_path_x1_x2_x3_x4 else (None, None)

        if model_dict_x4:
            # print("Loading x4 model...")
            # print(f"Before loading x4 (Model): {self.model.model_x4.x_embedder.proj.weight.mean().item()}")
            # print(f"Before loading x4 (EMA):   {self.ema.ema_model.model_x4.x_embedder.proj.weight.mean().item()}")
            missing, unexpected = self.model.model_x4.load_state_dict(model_dict_x4, strict=False)
            self.ema.ema_model.model_x4.load_state_dict(ema_dict_x4, strict=False)
            # print(f"After loading x4 (Model):  {self.model.model_x4.x_embedder.proj.weight.mean().item()}")
            # print(f"After loading x4 (EMA):    {self.ema.ema_model.model_x4.x_embedder.proj.weight.mean().item()}")
            # print(f"X4 Model Missing keys: {len(missing)}") # 应该是 0 或者很少
            # print(f"X4 Model Unexpected keys: {len(unexpected)}") 

            # 验证 Model 和 EMA 是否不同 (通常 EMA 和 Model 权重会有微小差异)
            # diff = (self.model.model_x4.x_embedder.proj.weight - self.ema.ema_model.model_x4.x_embedder.proj.weight).abs().sum()
            # print(f"Model vs EMA difference: {diff.item()}")

        if model_dict_x3_x4:
            print("Loading x3_x4 model...")
            missing, unexpected = self.model.model_x3_x4.load_state_dict(model_dict_x3_x4, strict=False)
            print(f"X34 Model Missing keys: {len(missing)}") # 应该是 0 或者很少
            print(f"X34 Model Unexpected keys: {len(unexpected)}") 
            
            self.ema.ema_model.model_x3_x4.load_state_dict(ema_dict_x3_x4, strict=False)
        if model_dict_x2_x3_x4:
            print("Loading x2_x3_x4 model...")
            missing, unexpected = self.model.model_x2_x3_x4.load_state_dict(model_dict_x2_x3_x4, strict=False)
            print(f"X234 Model Missing keys: {len(missing)}") # 应该是 0 或者很少
            print(f"X234 Model Unexpected keys: {len(unexpected)}") 
            
            self.ema.ema_model.model_x2_x3_x4.load_state_dict(ema_dict_x2_x3_x4, strict=False)
        if model_dict_x1_x2_x3_x4:
            print("Loading x1_x2_x3_x4 model...")
            missing, unexpected = self.model.model_x1_x2_x3_x4.load_state_dict(model_dict_x1_x2_x3_x4, strict=False)
            print(f"X1234 Model Missing keys: {len(missing)}") # 应该是 0 或者很少
            print(f"X1234 Model Unexpected keys: {len(unexpected)}") 
            
            self.ema.ema_model.model_x1_x2_x3_x4.load_state_dict(ema_dict_x1_x2_x3_x4, strict=False)
        print("##### 加载成功 #####")
    ...
    def inference_iter(
        ...
        with torch.no_grad():
            ...
            outputs = self.model.sample(*model_sampling_input_list)
