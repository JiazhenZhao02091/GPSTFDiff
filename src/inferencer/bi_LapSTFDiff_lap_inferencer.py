...
'''
    Bi-Directional STF Unet LAP Inferencer
    支持拉普拉斯金字塔多级推理 (x3, x2_x3, x1_x2_x3)，并适应双向输入 (c1, c2, c3, f1, f3)
'''
class Inferencer:
    def __init__(
        self,
        ...
        model=None,
        checkpoint_path_x3=None,
        checkpoint_path_x2_x3=None,
        checkpoint_path_x1_x2_x3=None,
        device_info=None,
        metric_list=None,
    ):
        ...
        # Initialize dictionary to hold multi-stage models
        self.model_dict = {}
        # Load state dicts for different stages but do not load into model yet
        if checkpoint_path_x3 is not None:
             self.model_dict["x3"] = self.get_dict(checkpoint_path_x3)
        if checkpoint_path_x2_x3 is not None:
             self.model_dict["x2_x3"] = self.get_dict(checkpoint_path_x2_x3)
        if checkpoint_path_x1_x2_x3 is not None:
             self.model_dict["x1_x2_x3"] = self.get_dict(checkpoint_path_x1_x2_x3)
    ...
    def get_dict(self, ckpt_path, ema_prefix = "ema_model.model."):
        checkpoint = torch.load(ckpt_path)
        # Handle EMA keys if necessary, or just load 'ema' key directly if consistent
    ...
    def init_model(self, model):
        self.device = 'cuda'
        # Model is just the architecture class here
    ...
    def before_inference_iter(self, data_per_batch):
        ...
        # Bi-Directional Visualization List: c1, c2, c3, f1, f2, f3
        show_img_list = [
            data_per_batch['coarse_img_01'],
            data_per_batch['coarse_img_02'],
            data_per_batch['coarse_img_03'], # Added
            data_per_batch['fine_img_01'],
            data_per_batch['fine_img_02'],
            data_per_batch['fine_img_03'],   # Added
        ]
    ...
    def get_model_sampling_input(self, data_per_batch):
        ...
        coarse_img_03 = data_per_batch['coarse_img_03'].to(self.device) # Added
        fine_img_01 = data_per_batch['fine_img_01'].to(self.device)
        fine_img_03 = data_per_batch['fine_img_03'].to(self.device)     # Added
        
        # Inputs for Bi-Directional model: c1, c2, c3, f1, f3
        model_sampling_input_list = [
            coarse_img_01, 
            coarse_img_02, 
            coarse_img_03, 
        ...
        if 'mask' in data_per_batch:
             mask = data_per_batch['mask'].to(self.device)
             model_sampling_input_list.append(mask)
    ...
    def inference_iter(
        ...
        with torch.no_grad():
            ...
            # LAP Logic: Iterate through loaded model stages
            # If model_dict is empty, this loop won't run, check logic if simple inference is needed.
            if not self.model_dict:
                 # Fallback if no specific stages provided (just run whatever is in model)
                 final_outputs = self.ema.ema_model.sample(*model_sampling_input_list)
            else:
                # Order matters: x3 -> x2_x3 -> x1_x2_x3 usually, or just run the most complex one?
                # Usually in LAP, you run coarse-to-fine or stage-by-stage.
                # Assuming here we simply want to run the keys present.
                for stage_name, state_dict in self.model_dict.items():
                    # Load weights for this stage
                    self.ema.load_state_dict(state_dict, strict=False)
                    # Run sampling
                    outputs = self.ema.ema_model.sample(*model_sampling_input_list)
                    final_outputs = outputs
                    # Note: If subsequent stages need previous output as input, 
                    # logic inside 'get_model_sampling_input' or here needs adjustment.
                    # Standard STF-Diff LAP inference usually just updates weights and re-runs sample 
                    # if the inputs are static context.

            if final_outputs is None:
                raise RuntimeError("No model output produced. Check checkpoint paths.")

            pixel_loss = F.mse_loss(final_outputs, loss_gt)
        ...
        # Visualization
        batch_num = final_outputs.shape[0]
        for batch_idx in range(batch_num):
            ...
            output = final_outputs[batch_idx : batch_idx + 1]
    ...
    def img_show(
        ...
        # 6 images (c1, c2, c3, f1, f2, f3) -> 3 rows, 2 columns?
        ...
        for h_index in range(h_num - 1):
            for w_index in range(w_num):
                ...
                if idx >= len(show_img_list): break
        ...
        # Place prediction result in the last row
        show_img[
