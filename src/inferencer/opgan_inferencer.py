...
class Inferencer:
    ...
    def init_tracker(self):
        key_list = ['pixel_loss'] + [metric.__name__ for metric in self.metric_list]
    ...
    def inference_iter(
        ...
        with torch.no_grad():
            phase_g_outputs, fine_temporal_change_img = self.model(
                *model_generator_input_list
            )
            pixel_loss = F.mse_loss(phase_g_outputs, loss_gt)

        self.inference_tracker.update('pixel_loss', pixel_loss.item())
        self.backend_logger.add_scalar(
            f'inference_running/pixel_loss',
            pixel_loss.item(),
        ...
        show_dir_prefix = (
            f'{dataset_name}/show_img/fine_temporal_change_img'
        )
        show_dir_path = self.inference_imgs_dir / show_dir_prefix
        show_name = key.split('-')[-1]
        show_name = (
