...
class Inferencer:
    ...
    def init_tracker(self):
        ...
        for stage in ['stage_1', 'stage_2']:
            for metric in self.metric_list:
                key_list.append(f'{stage}_{metric.__name__}')
    ...
    def inference(self):
        ...
        msg = f'Final Average Results (Epoch {self.current_epoch})'
    ...
    def inference_iter(
        ...
        with torch.no_grad():
            fine_img_stage_1 = self.cnn_stage_1(*cnn_stage_1_input_list)

            fine_img_stage_1_resize = F.interpolate(
                fine_img_stage_1, scale_factor=1 / 4, mode='nearest'
            )
            cnn_stage_2_input_list[1] = fine_img_stage_1_resize
            fine_img_stage_2 = self.cnn_stage_2(*cnn_stage_2_input_list)
        msg = f'inference iter: {iter_idx}, dataset: {dataset_name}'
        msg += '\nstage_1'
        for metric in self.metric_list:
            metric_value = metric(
                (fine_img_stage_1 + 1.0) / 2.0,
                (metrics_gt_stage_1 + 1.0) / 2.0,
            )
            metric_name = metric.__name__
            msg = msg + f', {metric_name}: {metric_value:.4f}'
            self.val_tracker.update(f'stage_1_{metric_name}', metric_value.item())

        msg += '\nstage_2'
        for metric in self.metric_list:
            metric_value = metric(
                (fine_img_stage_2 + 1.0) / 2.0,
                (metrics_gt_stage_2 + 1.0) / 2.0,
            )
            metric_name = metric.__name__
            msg = msg + f', {metric_name}: {metric_value:.4f}'
            self.val_tracker.update(f'stage_2_{metric_name}', metric_value.item())
        ...
        show_img_stage_2_list[1] = fine_img_stage_1_resize
        if 'STIL' in save_dir_prefix:
    ...
    def cnn_stage_1(
        ...
        fine_img_stage_1 = self.model_stage_1(
            coarse_img_01_stage_1,
            coarse_img_02_stage_1,
            coarse_img_03_stage_1,
            fine_img_01_stage_1,
    ...
    def cnn_stage_2(
        ...
        fine_img_stage_2 = self.model_stage_2(
            coarse_img_01_stage_2,
            coarse_img_02_stage_2,
            coarse_img_03_stage_2,
            fine_img_01_stage_2,
