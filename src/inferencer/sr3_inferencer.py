...
class Inferencer:
    ...
    def before_inference_iter(self, data_per_batch):
        ...
        gt = data_per_batch["fine_img"].to("cuda")
    ...
    def get_model_input(self, data_per_batch):
        coarse_img = data_per_batch["coarse_img"].to("cuda")

        fine_img = data_per_batch["fine_img"].to("cuda")

        coarse_img_patches = F.unfold(
            coarse_img, self.patch_size, stride=self.stride_size
        ).view(1, 6, self.patch_size, self.patch_size, -1)
        fine_img_patches = F.unfold(
            fine_img, self.patch_size, stride=self.stride_size
        ).view(1, 6, self.patch_size, self.patch_size, -1)
    ...
    def get_img_show_list(self, data_per_batch):
        coarse_img = data_per_batch["coarse_img"]
        fine_img = data_per_batch["fine_img"]
    ...
    def inference_iter(
        ...
        model_output_patches = torch.zeros_like(model_input_patches_list[0])
        cnt = torch.ones_like(model_input_patches_list[0])
        msg = (
            f"val epoch: {self.current_epoch}, iter: {iter_idx}, patch_num: {patch_num}"
        )
        print(patch_num)
        for patch_idx in range(patch_num):
            corase_img_patch = model_input_patches_list[0][..., patch_idx]
            ...
            model_output_patches[..., patch_idx] = model_output_patch

        model_output_patches = model_output_patches.view(1, -1, patch_num)
    ...
    def inference_patch(self, iter_idx, model_input_list):
        with torch.no_grad():
            ...
            # _, c, _, _ = model_input.shape
            model_input = model_input.transpose(0, 1)
            model_input = torch.repeat_interleave(model_input, 3, dim=1)

            model_output = self.model.sample(model_input)
            model_output = torch.mean(model_input, dim=1, keepdim=True)
