...
class Inferencer:
    def __init__(
        ...
        self.init_inference_dir(inference_root_dir_path, inference_dir_prefix_list, congfig_path)
    ...
    def init_inference_dir(self, inference_root_dir_path, inference_dir_prefix_list, congfig_path):
    ...
    def inference(self):
        ...
        for iter_idx, data_per_batch in enumerate(self.test_dataloader):
            ...
            self.inference_iter(iter_idx, model_input_list, gt, data_per_batch, show_img_list)
    ...
    def inference_iter(self, iter_idx, model_input_list, metrics_gt, data_per_batch, show_img_list):
        with torch.no_grad():
    ...
    def img_show(
        ...
        # show_img_path = os.path.join(str(show_dir_path), show_name)
        # show_dir_path.mkdir(parents=True, exist_ok=True)

        # 将字符串路径转换为Path对象
        show_dir_path_obj = Path(os.path.join(self.inference_imgs_dir, show_dir_path))
        show_dir_path_obj.mkdir(parents=True, exist_ok=True)
        
        show_img_path = os.path.join(str(show_dir_path_obj), show_name)
