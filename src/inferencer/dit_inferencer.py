...
class Inferencer:
    def __init__(
        ...
        self.init_model(model, checkpoint_path_x3, checkpoint_path_x2_x3, checkpoint_path_x1_x2_x3)
    ...
    # 初始化推理model 其内部拥有三个子《 去噪网络 》
    def init_model(self, model, checkpoint_path_x3, checkpoint_path_x2_x3, checkpoint_path_x1_x2_x3):
        self.device = 'cuda'
        self.model = model.to(self.device)
        self.ema = EMA(self.model, beta=0.995, update_every=1).to(self.device)
        
        model_dict_x3, ema_dict_x3 = self.get_model_ema_dict(checkpoint_path_x3) if checkpoint_path_x3 else (None, None)
        model_dict_x2_x3, ema_dict_x2_x3 = self.get_model_ema_dict(checkpoint_path_x2_x3) if checkpoint_path_x2_x3 else (None, None)
        model_dict_x1_x2_x3, ema_dict_x1_x2_x3 = self.get_model_ema_dict(checkpoint_path_x1_x2_x3) if checkpoint_path_x1_x2_x3 else (None, None)

        #TODO: 验证添加是否成功呢？
        if model_dict_x3:
            print("Loading x3 model...")
            self.model.model_x3.load_state_dict(model_dict_x3, strict=False)
            self.ema.ema_model.model_x3.load_state_dict(ema_dict_x3, strict=False)
            # print("Sample weight x3:", self.ema.ema_model.model_x3.init_conv.weight.sum().item())
        if model_dict_x2_x3:
            print("Loading x2_x3 model...")
            self.model.model_x2_x3.load_state_dict(model_dict_x2_x3, strict=False)
            self.ema.ema_model.model_x2_x3.load_state_dict(ema_dict_x2_x3, strict=False)
            # print("Sample weight x2_x3:", self.ema.ema_model.model_x2_x3.init_conv.weight.sum().item())
        if model_dict_x1_x2_x3:
            print("Loading x1_x2_x3 model...")
            self.model.model_x1_x2_x3.load_state_dict(model_dict_x1_x2_x3, strict=False)
            self.ema.ema_model.model_x1_x2_x3.load_state_dict(ema_dict_x1_x2_x3, strict=False)
            # print("Sample weight x1_x2_x3:", self.ema.ema_model.model_x1_x2_x3.init_conv.weight.sum().item())
        print("##### 加载成功 #####")
