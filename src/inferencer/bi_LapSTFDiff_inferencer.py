...
'''
    Bi-Directional STF Unet Inferencer
    支持输入: coarse_01, coarse_02, coarse_03, fine_01, fine_03
'''
class Inferencer:
    def __init__(
        self,
    ...
    def before_inference_iter(self, data_per_batch):
        ...
        # Bi-Directional Visualization List
        show_img_list = [
    ...
    def get_model_sampling_input(self, data_per_batch):
        ...
        # Bi-Directional Inputs: c1, c2, c3, f1, f3
        # Depending on your BiGaussianDiffusion.sample signature, you might need mask too
        # But typically for inference:
        model_sampling_input_list = [
            coarse_img_01, 
            coarse_img_02, 
            coarse_img_03, 
        ...
        # NOTE: If your bi_diffusion.sample() expects a mask argument (even if None), 
        # include it here. For now, assuming standard bi_diffusion signature.
    ...
    def inference_iter(
        ...
        for metric in self.metric_list:
            ...
            self.backend_logger.add_scalar(
                f'val_runinng/{metric_name}', metric_value.item(), self.current_inference_step
            )
    ...
    def img_show(
        ...
        # 调整布局以适应双向输入的 6 张图 (c1,c2,c3, f1,f2,f3) -> 3行, 每行2张? 或者 2行3列
        # show_len usually 6 now.
        # h_num=3, w_num=show_len//2=3 ? 
        # Default Logic: h_num=3 (Input_Row, Target_Row, Pred_Row?)
        # Let's keep logic simple:
        
        h_num, w_num = 3, show_len // 2
        # If show_len is 6 (c1, c2, c3, f1, f2, f3), w_num = 3.
        # This will create a grid of inputs.
        ...
        # Place prediction on the 3rd row, centered or first slot
        show_img[
