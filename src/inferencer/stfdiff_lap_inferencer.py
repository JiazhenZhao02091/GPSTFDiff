...
class Inferencer:
    ...
    def get_dict(self, ckpt_path, ema_prefix = "ema_model.model."):
        """
        传入包含ema的checkpoints 返回出去掉ema前缀的state_dict
        可以直接赋值到model中去
        :param ckpt_path: checkpoints路径
        :param ema_prefix: 去掉的前缀 默认即可
        """
        ...
        for key, value in ema_ckpt.items():
            if key.startswith(ema_prefix):
                # "ema_model.model.blocks.0..." -> "blocks.0..."
                new_key = key[len(ema_prefix):] 
                new_state_dict[new_key] = value
        if not new_state_dict:
            self.txt_logger.warning(f"Warning: No keys found with prefix '{ema_prefix}' in {ckpt_path}. Check your prefix or checkpoint structure.")
    ...
    def init_model(self, model, checkpoint_path_x3, checkpoint_path_x2_x3, checkpoint_path_x1_x2_x3):
        ...
        ckpt_dict_x3 = self.get_dict(checkpoint_path_x3)
        ckpt_dict_x2_x3 = self.get_dict(checkpoint_path_x2_x3)
        ckpt_dict_x1_x2_x3 = self.get_dict(checkpoint_path_x1_x2_x3)

        res_x3 = self.model.model_x3.load_state_dict(ckpt_dict_x3, strict=False)
        res_x2_x3 = self.model.model_x2_x3.load_state_dict(ckpt_dict_x2_x3, strict=False)
        res_x1_x2_x3 = self.model.model_x1_x2_x3.load_state_dict(ckpt_dict_x1_x2_x3, strict=False)

        self.txt_logger.info(f"模型检查点加载完成")
        # 分别打印三个模型的加载情况
        self.txt_logger.info(f"model_x3 - missing: {len(res_x3.missing_keys)}, unexpected: {len(res_x3.unexpected_keys)}")
        self.txt_logger.info(f"model_x2_x3 - missing: {len(res_x2_x3.missing_keys)}, unexpected: {len(res_x2_x3.unexpected_keys)}")
        self.txt_logger.info(f"model_x1_x2_x3 - missing: {len(res_x1_x2_x3.missing_keys)}, unexpected: {len(res_x1_x2_x3.unexpected_keys)}")
