...
class ErrorRelativeGlobalDimensionlessSynthesis(nn.Module):
    ...
    def forward(self, gt, pred, mask=None):
        ...
        if mask is not None:
             ...
             # 1. 计算每个通道的 MSE (Sum / N)
             mse_per_channel = torch.sum(diff * diff, dim=(-2, -1)) / N
             
             # 2. 计算每个通道 GT 的均值 (Sum / N) 及均值的平方
             mu_gt_per_channel = torch.sum(gt, dim=(-2, -1)) / N
             mu_gt_sq_per_channel = mu_gt_per_channel.pow(2)
        else:
             diff = gt - pred
        ...
        ergas_batch = (
            100
            * self.ratio
            * torch.mean(mse_per_channel / (mu_gt_sq_per_channel + 1e-10), dim=1).sqrt()
        )
