...
def _get_backbone_dino():
    ...
    #TODO: 修改模型的输入通道
    backbone.patch_embed.proj = nn.Conv2d(6, 1024, kernel_size=(16, 16), stride=(16, 16))
...
class TransformerEncoder(nn.Module):
    """
        Layers + norm
    """
    def __init__(self, encoder_layer, num_layers, norm=None, max_seq_len=256, d_model = 1024):
        super().__init__()
        ...
        # 使用正弦位置编码进行初始化
        pos_embed = self._get_sinusoidal_pos_embed(max_seq_len, d_model)
    ...
    def forward(self, src,
        ...
        # TODO 位置编码
        B, N, _ = output.shape
        assert N <= self.max_seq_len, f"序列长度 {N} 超过了最大长度 {self.max_seq_len}"
        output = output + self.pos_embed[:, :N, :]
...
class Dino(nn.Module):
    def __init__(self, backbone=None, d_model=1024, n_heads=8, 
        ...
        # Encoder
        self.encoder_layer = TransformerEncoderLayer(d_model=d_model, nhead=n_heads, activation="silu")
        self.encoder = TransformerEncoder(self.encoder_layer, num_encoder_layers, encoder_norm)
        
        # Decoder
        self.decoder_layer = TransformerDecoderLayer(d_model=d_model, nhead=n_heads, activation="silu")
    ...
    def norm(x):
        x = self.n(x)
        return x
    def forward(self, coarse_1, coarse_2, fine_1, fine_2): # TODO: fine_2 可以忽略
        # 6 256 256
        ...
        time_fea = self.backbone.forward_features(time_diff)[:, 5:, :]      # 1 256 1024
        sensor_fea = self.backbone.forward_features(sensor_diff)[:, 5:, :]  # 1 256 1024
