...
class EDMDiffusion(nn.Module):
    ...
    def get_xt(self, x0_t, t, scale=1):
        # sqrt(1 - alpha^)
        epsilon = extract(self.sqrt_one_minus_alphas_cumprod, t, x0_t.shape) * torch.randn_like(x0_t) * scale
        x_t = x0_t + epsilon
        return x_t, epsilon
    ...
    def p_losses(self, coarse_img_01, coarse_img_02, fine_img_01, fine_img_02, t):
        ...
        " 计算x_t "
        x_t, epsilon = self.get_xt(x0_t, t)
        ...
        # )
        " 损失值 "
        loss = self.loss_fn(outputs, epsilon)  # prediction type is "sample"
