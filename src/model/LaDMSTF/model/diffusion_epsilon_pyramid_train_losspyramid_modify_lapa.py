...
class EDMDiffusion(nn.Module):
    ...
    def laplacian_pyramid(self, img, levels=3):
        ...
        for i in range(levels):
            ...
            pyramid.append(img - up)  # TODO 这里金字塔分解是否正确？
            img = down
        pyramid.append(img)
    ...
    def p_losses(self, coarse_img_01, coarse_img_02, fine_img_01, fine_img_02, t):
        ...
        " 计算x_t "
        noisy_image = torch.randn_like(fine_img_02)
        noise_1, noise_2, noise_3 = self.laplacian_pyramid(noisy_image, levels=3)
        noise_3_up = F.interpolate(noise_3, scale_factor=2, mode='bilinear', align_corners=False)
        noise_2_3_up = F.interpolate(noise_2 + noise_3_up, scale_factor=2, mode='bilinear', align_corners=False)
        if self.mode == "x3":
            x_t = x0_t + extract(self.sqrt_one_minus_alphas_cumprod, t, x0_t.shape) * noise_3
        elif self.mode == "x2+x3":
            x_t = x0_t + extract(self.sqrt_one_minus_alphas_cumprod, t, x0_t.shape) * (noise_2 + noise_3_up)
        else:  # x1 + x2 + x3
            # print(f"xt shape is {x0_t.shape}")
            x_t = x0_t + extract(self.sqrt_one_minus_alphas_cumprod, t, x0_t.shape) * (noise_1 + noise_2_3_up)
    ...
    def p_sample_loop(
        ...
        for t in tqdm.tqdm(
            ...
            if t >= self.sample_T2:
                ...
                if t == self.sample_T2:
                    x0_t = F.interpolate(x0_t, scale_factor=2, mode='bilinear', align_corners=False)
                    xt = self.get_xt(x0_t, batched_times, model=2, noise=noise_2, scale=2)  # xt = 
                else:
                ...
                if t == self.sample_T1:
                    x0_t = F.interpolate(x0_t, scale_factor=2, mode='bilinear', align_corners=False)
                    xt = self.get_xt(x0_t, batched_times, model=1, noise=noise_1, scale=2)
                else:
                    xt = self.get_xt(x0_t, batched_times)
            else:
                self.mode = 'x1+x2+x3'
