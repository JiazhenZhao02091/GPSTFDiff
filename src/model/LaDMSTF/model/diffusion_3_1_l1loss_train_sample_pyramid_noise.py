...
class EDMDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.T1 = T1       # t1 interval
        self.T2 = T2       # t2 interval
    ...
    def generate_decay_tensors_linear(self, t1: int, t2: int, total_steps: int = 1000, initial_value: float = 1.0):
        ...
        def get_decay(duration):
            ...
            # 截断：超过 duration 的部分设为 0，且数值不能小于 0
    ...
    def get_epsilon(self, noise_1, noise_2, noise_3, t):
        sigma_t_1 = extract(self.sqrt_one_minus_alphas_cumprod, t, noise_1.shape)
        sigma_t_2 = extract(self.sqrt_one_minus_alphas_cumprod, t, noise_2.shape)
        sigma_t_3 = extract(self.sqrt_one_minus_alphas_cumprod, t, noise_3.shape)
        
        x_epsilon = noise_3
        if 'x2' in self.mode:
            x_epsilon = F.interpolate(x_epsilon, scale_factor=2, mode='bilinear', align_corners=False)
            x_epsilon = x_epsilon + noise_2
        if 'x1' in self.mode:
            x_epsilon = F.interpolate(x_epsilon, scale_factor=2, mode='bilinear', align_corners=False)
            x_epsilon = x_epsilon + noise_1
        
        if self.mode == 'x3':
            return x_epsilon * sigma_t_3
        elif self.mode == 'x2+x3':
            return x_epsilon * sigma_t_2
        else:
            return x_epsilon * sigma_t_1
    
    def get_xt(self, x0_t, t, scale=1, noise = None):
        # noise = epsilon = sigma_t * x_epsilon 
        if noise is None:
            x_t = x0_t + extract(self.sqrt_one_minus_alphas_cumprod, t, x0_t.shape) * torch.randn_like(x0_t) * scale
        else:
            x_t = x0_t + noise * scale
        return x_t
    ...
    def p_losses(self, coarse_img_01, coarse_img_02, fine_img_01, fine_img_02, t):
        ...
        noise_1, noise_2, noise_3 = self.laplacian_pyramid(noise, levels=3)
        " 计算epsilon "
        epsilon = self.get_epsilon(noise_1, noise_2, noise_3, t)
        " 计算x0_t "
        x0_t = self.get_x0_t(x1, x2, x3, t)  
        " 计算x_t "
        x_t = self.get_xt(x0_t, t, noise=epsilon)
    ...
    def p_sample_loop(
        ...
        noise_1, noise_2, noise_3 = self.laplacian_pyramid(xt, levels=3)
        xt = noise_3
        for t in tqdm.tqdm(
            ...
            if t >= self.sample_T2:
                ...
                if t == self.sample_T2:
                    x0_t = F.interpolate(x0_t, scale_factor=2, mode='bilinear', align_corners=False)
                    xt = self.get_xt(x0_t, batched_times, noise=noise_2, scale=2)  # xt = 
                else:
                ...
                if t == self.sample_T1:
                    x0_t = F.interpolate(x0_t, scale_factor=2, mode='bilinear', align_corners=False)
                    xt = self.get_xt(x0_t, batched_times,noise=noise_1, scale=2)
                else:
                    xt = self.get_xt(x0_t, batched_times)
            else:
                self.mode = 'x1+x2+x3'
