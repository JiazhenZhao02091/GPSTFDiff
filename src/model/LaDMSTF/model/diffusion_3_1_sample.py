...
class EDMDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.T1 = 200       # t1 interval
        self.T2 = 500       # t2 interval
        self.sample_T1 = (self.T1 * sampling_timesteps // num_train_timesteps) if self.T1 > 250 else 15
    ...
    def init_para(self, beta_schedule='cosine', timesteps=1000):
        ...
        alpha_3, alpha_1, alpha_2 = self.generate_decay_tensors(t1 = self.T1, t2 = self.T2, total_steps = timesteps)
    ...
    def sample(self, coarse_img_01, coarse_img_02, fine_img_01):
        noisy_fine_img_02 = torch.rand_like(fine_img_01).to(fine_img_01.device)

        latents = noisy_fine_img_02
        condition = [coarse_img_01, coarse_img_02, fine_img_01]

        return self.edm_sampler_multi_stage(
            net=self.model, 
            latents=latents, 
            condition=condition,
        )
    ...
    " sampler "
    @torch.no_grad()
    def edm_sampler_multi_stage(
        self,
        net, latents, condition=None, randn_like=torch.randn_like,
        num_steps=18, sigma_min=0.002, sigma_max=80, rho=7,
        S_churn=0, S_min=0, S_max=float('inf'), S_noise=1,
    ):
        coarse_img_01, coarse_img_02, fine_img_01 = condition[0], condition[1], condition[2]
        noise_1, noise_2, noise_3 = self.laplacian_pyramid(latents, levels=3)
        t2 = num_steps // 2 # 9
        t1 = t2 // 2  + t2 # 4 + 9 = 13
        ...
        # Main sampling loop.
        x_next = noise_3.to(torch.float32) * t_steps[0]
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
            ...
            # Euler step.
            denoised = net(x_hat, t_hat, condition).to(torch.float32)
            d_cur = (x_hat - denoised) / t_hat
            x_next = x_hat + (t_next - t_hat) * d_cur

            # Apply 2nd order correction.
            if i < num_steps - 1:
                denoised = net(x_next, t_next, condition).to(torch.float32)
                d_prime = (x_next - denoised) / t_next
                x_next = x_hat + (t_next - t_hat) * (0.5 * d_cur + 0.5 * d_prime)
            
            if i == (t2 - 1):
                x_next = F.interpolate(x_next, scale_factor=2, mode='bilinear', align_corners=False)
                # x_next = x_next + torch.randn_like(x0_t)
                x_next = x_next + noise_2
            if i == (t1 - 1):
                x_next = F.interpolate(x_next, scale_factor=2, mode='bilinear', align_corners=False)
                # x_next = x_next + torch.randn_like(x0_t)
                x_next = x_next + noise_1
        return x_next
