...
"""
    Diffusion model for Laplacian pyramid single model
    只训练以及采样单一一个大模型，也就是三个阶段全部使用一个模型
"""
...
class Diffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.sample_T1 = round(T1 / num_train_timesteps * sampling_timesteps)
        self.sample_T2 = round(T2 / num_train_timesteps * sampling_timesteps)
    ...
    def init_para(self, beta_schedule='cosine', timesteps=1000):
        ...
        debug_idx = min(400, timesteps - 1)
        print(f"alpha_3[{debug_idx}]: {alpha_3[debug_idx]}, alpha_1[{debug_idx}]: {alpha_1[debug_idx]}, alpha_2[{debug_idx}]: {alpha_2[debug_idx]}")
    ...
    def compose_pyramid_by_mode(self, x1, x2, x3):
        if self.mode == "x3":
            return x3
        elif self.mode == "x2+x3":
            x3 = cv2_interpolate(x3, scale_factor=2, mode='bilinear', align_corners=False)
    ...
    def get_noise_target(self, x0_t, fine_img_02, noise=None):
        if self.noise_target == 'standard':
            return default(noise, lambda: torch.randn_like(x0_t))

        if self.noise_target == 'laplacian':
            full_noise = default(noise, lambda: torch.randn_like(fine_img_02))
            n1, n2, n3 = self.laplacian_pyramid(full_noise, levels=3)
            return self.compose_pyramid_by_mode(n1, n2, n3)

        raise ValueError(f'unknown noise_target {self.noise_target}')
    ...
    def p_losses(self, coarse_img_01, coarse_img_02, fine_img_01, fine_img_02, t, noise=None):
        ...
        noise = self.get_noise_target(x0_t, fine_img_02, noise=noise)
    ...
    def forward(self, coarse_img_01, coarse_img_02, fine_img_01, fine_img_02):
        ...
        # 随机选择当前 batch 的训练模式
        # 这样模型就能同时学习三个阶段的去噪能力
        rand_mode = torch.rand(1).item()
        
        if rand_mode < 0.333:
            self.mode = "x1+x2+x3"
            t = torch.randint(0, self.T1, (b,), device=device).long()
        elif rand_mode < 0.666:
            self.mode = "x2+x3"
