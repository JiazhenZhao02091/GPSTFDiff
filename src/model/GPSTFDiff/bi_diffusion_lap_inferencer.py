...
"""
    Bi-Diffusion Lap Inferencer
    Combined from diffusion_lap_inferencer.py and bi_diffusion.py
"""
...
class BiDiffusionLapInferencer(nn.Module):
    def __init__(
        self,
        model,
        *,
        ...
        self.self_condition = False # Model in bi_diffusion has self_condition attribute, but here we default to False or check model later
        if hasattr(model, 'self_condition'):
    ...
    def masked_l1_loss(self, pred, target, mask):
        ...
        # 3. 在 c, h, w 维度上求和，但在 b 维度上保持
        # sum 结果维度为 (b,)
        sum_loss = torch.sum(masked_diff, dim=(1, 2, 3))
        # 4. 计算每个 batch 样本的有效像素数 (b,)
        # 每个样本的有效像素 = mask中1的数量 * 通道数
        num_valid = torch.sum(mask, dim=(1, 2, 3)) * pred.size(1)
        # 5. 返回每个样本的平均 loss (b,)
    ...
    def model_predictions(
        ...
        # Apply condition scaling based on current mode
        c_img_01, c_img_02, c_img_03, f_img_01, f_img_03 = self.get_condition(
            coarse_img_01, coarse_img_02, coarse_img_03, fine_img_01, fine_img_03
        )
    ...
    def p_losses(
        ...
        if mask is not None:
             loss = self.masked_l1_loss(model_out, target, mask)
        else:
    ...
    def ddim_sample(
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            # Using model_predictions with correct arguments for bi-diffusion
    ...
    def sample(
        ...
        # Initialize noisy image at smallest scale (1/4 size)
