...
"""
    三阶段 Diffusion 推理模型
    指定三阶段的 checkpoint 进而实现三阶段的采样过程
"""
...
class Diffusion(nn.Module):
    ...
    def model_predictions(
        ...
        coarse_img_01, coarse_img_02, fine_img_01 = torch.chunk(condition, 3, dim=1)
        # print(f"coarse_img_01.shape: {coarse_img_01.shape}, coarse_img_02.shape: {coarse_img_02.shape}, fine_img_01.shape: {fine_img_01.shape}")
    ...
    def ddim_sample(
        ...
        # 在 tqdm 外部套上 enumerate 方便提取当步 step
        for step, (time, time_next) in enumerate(tqdm(time_pairs, desc='sampling loop time step')):
            ...
            if time_next < 0:
                ...
                # --- 保存最后一步（如果落在间隔上） ---
                self._save_mid_img(
                    step, x_start, save_interval, save_dir, 
                    normalize_scale, normalize_mode, save_name_prefix
                )
                continue
    ...
    # 为了避免在多处 `continue` 中写重复的保存代码，我们提取成一个辅助函数
    def _save_mid_img(
        self, step, save_tensor, save_interval, save_dir, 
        normalize_scale, normalize_mode, save_name_prefix
    ):
        if self.save_interval is None or save_dir is None:
            return
        ...
        if (step + 1) % self.save_interval == 0 or step == 0:
            ...
            # 使用 self.mode (例如 x3, x2+x3, x1+x2+x3) 区分模型所处的尺度
            stage_str = self.mode.replace("+", "_")
            save_name = f"{save_name_prefix}_{stage_str}_step{step+1}.tif"
    ...
    def sample(
        ...
        return self.ddim_sample(
            ...
            noisy_fine_img_02,
            clip_denoised=True,
            save_interval=self.save_interval,  # 使用成员变量
