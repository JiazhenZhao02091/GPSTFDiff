...
class Diffusion(nn.Module):
    ...
    def generate_decay_tensors_cosine(
        ...
        if not (1 <= t1 <= total_steps):
            raise ValueError(f"t1 必须在 [1, {total_steps}] 范围内。")
        ...
        list2[mask_2] = 0.0
        # Only two stages, remove decay_duration_3/list3
        return list1, list2
    ...
    def init_para(self, beta_schedule='cosine', timesteps=1000):
        ...
        # Only two-stage now
        alpha_3, alpha_1 = self.generate_decay_tensors_cosine(t1 = self.T1, total_steps = timesteps)
        alpha_2 = None  # Use only alpha_1 and alpha_3

        register_buffer('alpha_1', alpha_1)
        # For compatibility, set alpha_2 to zeros
        register_buffer('alpha_2', torch.zeros_like(alpha_1))
        register_buffer('alpha_3', alpha_3)
        print(f"alpha_3[400]: {alpha_3[400]}, alpha_1[400]: {alpha_1[400]}")
    ...
    def get_target(self, fine_img_02):
        ...
        if self.mode == "x1+x2+x3":
            ...
            return x2 + cv2_interpolate(x3, scale_factor=2, mode='bilinear', align_corners=False)
        else: # x3
            return x3
    ...
    def forward(self, coarse_img_01, coarse_img_02, fine_img_01, fine_img_02):
        ...
        # Only two possible modes for two stages
        if self.mode == "x1+x2+x3":
    ...
    def ddim_sample(
        ...
        # Only two-stage sampling here, remove T2 phase
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            # Only one stage switch, from T3 (x3) to T1 (x1+x2+x3)
            i = i - 1
            if i == self.sample_T1:
                print(f"Sampling stage change: T3 -> T1 at step {i}")
