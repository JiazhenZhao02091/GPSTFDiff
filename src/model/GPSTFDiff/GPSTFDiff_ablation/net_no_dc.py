...
"""
    forward参数为 [coarse_img_01, coarse_img_02, fine_img_01, noisy_fine_img_02, time, x_self_cond]
"""
...
class PredNoiseNetMKIRA(nn.Module):
    def __init__(
        ...
        include_dc_in_cond: bool = True, # <--- [新增] 支持控制是否包含 dc
        # ===== MKIRA options =====
        use_mkira: bool = True,
        ...
        self.include_dc_in_cond = include_dc_in_cond

        # <--- [修改] 动态计算 conditional stream 的通道数
        cond_in_ch = channels * 2 # 基础包含 c1, c2
        if self.include_dc_in_cond:
            cond_in_ch += channels
    ...
    def forward(
        ...
        # <--- [修改] 动态构造 conditional tensor
        cond_list = [coarse_img_01, coarse_img_02]
...
if __name__ == "__main__":
    # 测试用例 1: 包含所有条件 (默认)
    print("Test 1: Full conditions (c1, c2, dc, f1)")
    net_full = PredNoiseNetMKIRA(
        dim=64, channels=6, out_dim=6, dim_mults=(1, 2, 4),
        include_f1_in_cond=True, include_dc_in_cond=True
    )
    
    # 测试用例 2: 不包含 dc 和 f1
    print("Test 2: No dc, no f1 (only c1, c2)")
    net_minimal = PredNoiseNetMKIRA(
        dim=64, channels=6, out_dim=6, dim_mults=(1, 2, 4),
        include_f1_in_cond=False, include_dc_in_cond=False
    )
    ...
    out_full = net_full(c1, c2, f1, noisy_f2, time)
    out_minimal = net_minimal(c1, c2, f1, noisy_f2, time)
    
    print(f"Full out shape: {out_full.shape}")
    print(f"Minimal out shape: {out_minimal.shape}")