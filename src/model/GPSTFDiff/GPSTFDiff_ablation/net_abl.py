...
"""
    统一消融网络 (Ablation Network)
    支持通过传参自由控制 MKIRA 中的 CA、SA、以及 MFE 模块
"""
...
class MKIRA_GN_Abl(nn.Module):
    """
    统一消融版本 MKIRA：
    包含 use_ca 和 use_sa 参数控制是否前传 Attention。
    """
    def __init__(self, c, kernel_sizes=(1, 3, 5), gn_groups=8, use_ca=True, use_sa=True):
        super().__init__()
        self.use_ca = use_ca
        self.use_sa = use_sa
...
# --------------------------
# main model (FiLM baseline + parameterized MKIRA refinement)
# --------------------------
class PredNoiseNetMKIRA_Abl(nn.Module):
    def __init__(
        self,
        dim: int = 64,
        init_dim: int = None,
        ...
        # ===== MKIRA Ablation options =====
        use_mkira: bool = True,
        use_ca: bool = True,      # CA 消融开关
        use_sa: bool = True,      # SA 消融开关
        use_mfe: bool = True,     # MFE 消融开关（False则退化为单核(3,)）
        ...
        # MFE消融策略：如果用MFE，就使用配置的多核，否则降级为经典的单通道提取(3,)
        actual_kernel_sizes = mkira_kernel_sizes if use_mfe else (3,)
            ...
            if self.use_mkira and (up_idx in set(mkira_up_indices)):
                self.mkira_refiners.append(
                    ResidualGated(
                        MKIRA_GN_Abl(
                            c=dim_out_, 
                            kernel_sizes=actual_kernel_sizes, 
                            gn_groups=mkira_gn_groups, 
                            use_ca=use_ca, 
                            use_sa=use_sa
                        ),
