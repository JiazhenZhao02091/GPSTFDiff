...
# # 1) oflex
# try:
#     import selective_scan_cuda_oflex as _ss_oflex
#     selective_scan_cuda_oflex = _ss_oflex
# except Exception:
#     pass

# # 2) core（可选；若没有则回退到 oflex）
# try:
#     import selective_scan_cuda_core as _ss_core
#     selective_scan_cuda_core = _ss_core
# except Exception:
#     selective_scan_cuda_core = selective_scan_cuda_oflex  # fallback

# # 3) selective_scan_cuda（mamba版；若没有则回退到 core/oflex）
# try:
#     import selective_scan_cuda as _ss_cuda
#     selective_scan_cuda = _ss_cuda
# except Exception:
#     selective_scan_cuda = selective_scan_cuda_core  # fallback

# # 最终兜底：三个都没有就直接报错，避免后面静默出错
# if selective_scan_cuda_oflex is None and selective_scan_cuda_core is None and selective_scan_cuda is None:
#     raise ImportError(
#         "Cannot import any selective_scan CUDA extensions. "
#         "Tried: selective_scan_cuda_oflex / selective_scan_cuda_core / selective_scan_cuda."
#     )
...
class SS2D(nn.Module):
    ...
    def __initv2__(
        ...
        **kwargs,    
        ...
        # fallback: if user asks v2 but core backend missing, use v3/oflex
        if forward_type == "v2" and selective_scan_cuda_core is None:
            self.forward_core = FORWARD_TYPES["v3"]
        k_group = 4
