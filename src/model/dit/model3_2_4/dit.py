...
from einops import rearrange # 需要确保安装了 einops
...
class SPCSA(nn.Module):
    """
    Sparse Pyramid Channel Self Attention (SPCSA) adapted for DiT.
    Input/Output: (B, N, C)
    Internal: Converts to (B, C, H, W) for Conv operations.
    """
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., **kwargs):
        ...
        # 1x1 Conv for local context / projection
        self.linear_0 = nn.Conv2d(dim, dim , 1, 1, 0)
        
        # QKV Generation: 1x1 Conv followed by 3x3 Depthwise Conv (Local Bias)
        ...
        # Dynamic Gate for Sparsity (Top-K selection)
    ...
    def forward(self, x, freqs_cis=None): 
        # x: (B, N, C)
        # freqs_cis: Ignored in SPCSA as it uses Conv2d with implicit positional bias, 
        # but kept for interface compatibility with DiTBlock.
        ...
        if s * s == N:
            ...
            # Handle 2x stacked sequence (e.g. x + condition)
            # N = 2 * w * w -> w = sqrt(N/2)
            w = int(math.sqrt(N / 2))
            if w * w * 2 == N:
                W = w
                ...
                raise ValueError(f"SPCSA: Input sequence length {N} is not supported (not square or 2x square).")
        
        # Reshape to Image format: (B, N, C) -> (B, C, H, W)
        ...
        # Rearrange for Channel Attention within Heads
        # q, k, v shape: (B, Heads, C_head, HW)
        ...
        # Calculate Dynamic K based on input features
        # gate output is (B, 1, H, W) -> mean over spatial -> (B, 1)
        gate_score = self.gate(x).view(B, -1).mean(dim=1, keepdim=True) 
        
        # Attention Map: (B, Heads, C_head, C_head) - This is Channel Attention
        attn = (q @ k.transpose(-2, -1)) * self.temperature
        
        # --- Sparse Top-K Mechanism ---
        C_head = q.shape[2]
        dynamic_k = int(C_head * gate_score.mean().item()) # Use mean gate score for batch simplicity or per sample
        dynamic_k = max(1, min(dynamic_k, C_head))
        
        # Select Top-K relevant channels
        topk_val, topk_idx = torch.topk(attn, k=dynamic_k, dim=-1, largest=True)
        mask_val = torch.full_like(attn, float('-inf'))
        attn = mask_val.scatter(-1, topk_idx, topk_val)

        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        # Aggregate: (B, Heads, C_head, C_head) @ (B, Heads, C_head, HW) -> (B, Heads, C_head, HW)
        out = (attn @ v)
        
        # --- Fix: Standard Multi-Head Concatenation ---
        # Instead of weighted sum of identical outputs, we rearrange back to full channels
        out = rearrange(out, 'b head c (h w) -> b (head c) h w', head=self.num_heads, h=H, w=W)
        
        out = self.project_out(out)
        # --- End SPCSA Core Logic ---

        # Reshape back to Sequence format: (B, C, H, W) -> (B, N, C)
    ...
    def forward(self, hidden_states: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        for i, module in enumerate(self.net):
            ...
            if i == 1:
                ...
                # Determine H and W (Same logic as SPCSA)
                s = int(math.sqrt(N))
                if s * s == N:
                    h = w = s
                else:
                    ...
                    if w * w * 2 == N:
                        ...
                        raise ValueError(f"FeedForwardControl: Input sequence length {N} is not supported.")
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, dropout=0.0,  drop_path=0.0,**block_kwargs):
        ...
        # [MODIFIED] Replaced FlashAttention with SPCSA
        # self.attn = FlashAttention(hidden_size, num_heads=num_heads, qkv_bias=True, attn_drop=dropout,  proj_drop=dropout, **block_kwargs)
        self.attn = SPCSA(hidden_size, num_heads=num_heads, qkv_bias=True, attn_drop=dropout, proj_drop=dropout, **block_kwargs)
    ...
    def forward(self, x, c_t, freqs_cis=None): # c_img = None
        ...
        # [MODIFIED] SPCSA is injected here using the same adaLN modulation (shift_msa, scale_msa) and gating (gate_msa)
