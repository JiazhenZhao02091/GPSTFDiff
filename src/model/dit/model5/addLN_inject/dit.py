...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c_t, c_img = None):
        ...
        # x = x + self.mlp_2(self.norm_ffc(x)) # (N, T, D)
        return x
...
class HaarWaveletTransform(nn.Module):
    def __init__(self, in_channels=6):
        ...
        haar_weights = torch.tensor([
            ...
            [[1, -1], 
             [-1, 1]]  # HH
        ], dtype=torch.float32).unsqueeze(1) * 0.5  # <--- 关键修改：增加 unsqueeze(1)
        self.register_buffer('weight', haar_weights.repeat(in_channels, 1, 1, 1))
    ...
    def forward(self, x):
        ...
        out = out.permute(0, 2, 1, 3, 4).reshape(B, self.in_channels * 4, H, W)
...
class InverseHaarWaveletTransform(nn.Module):
    ...
    def forward(self, x):
        B, _, H, W = x.shape
        x = x.reshape(B, 4, self.in_channels, H, W)
        x = x.permute(0, 2, 1, 3, 4).reshape(B, self.in_channels * 4, H, W)
        
        out = F.conv_transpose2d(x, self.weight, stride=2, groups=self.in_channels)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        # num_classes=1000,
        learn_sigma=False,
        wavelet_levels=2 # Edify Image 使用了 2 级小波
    ):
        super().__init__()
        ...
        for level in range(wavelet_levels):
            self.dwt_list.append(HaarWaveletTransform(in_channels=current_channels))
            self.iwt_list.append(InverseHaarWaveletTransform(in_channels=current_channels))
            current_channels = current_channels * 4
        # 计算变换后的通道数 每级变换通道数 x4
        self.feat_channels = in_channels * (4 ** wavelet_levels) 
        self.feat_size = input_size // (2 ** wavelet_levels)

        # 修改Embedding层
        self.x_embedder = nn.Linear(self.feat_channels, hidden_size)
        # TODO 这里暂时设置条件和图像的形状一样
        self.cond_embedder = nn.Linear(self.feat_channels, hidden_size) # 假设condition也是同样处理
        self.t_embedder = TimestepEmbedder(hidden_size)
        # 位置编码
        num_patches = (self.feat_size // patch_size) ** 2
        ...
        # 输出维度直接映射回小波通道数
        self.final_linear = nn.Linear(hidden_size, self.feat_channels, bias=True) # 这里假设不预测sigma，简化展示
        # Cond
    ...
    def initialize_weights(self):
        ...
        cond_pos_embed = get_2d_sincos_pos_embed(self.cond_pos_embed.shape[-1], int(self.num_patches ** 0.5))
        self.cond_pos_embed.data.copy_(torch.from_numpy(cond_pos_embed).float().unsqueeze(0))

        # Initialize x_embedder (nn.Linear):
        nn.init.xavier_uniform_(self.x_embedder.weight)
        nn.init.constant_(self.x_embedder.bias, 0)

        # Initialize cond_embedder (nn.Linear):
        nn.init.xavier_uniform_(self.cond_embedder.weight)
    ...
    def unpatchify_wavelet(self, x):
        """
        x: (N, T, hidden_size) -> 逆变换 -> (N, C, H, W)
        """
        N, T, C = x.shape
        h = w = int(T ** 0.5)
    ...
    def forward(self, x, t, condition):
        ...
        for level in range(self.wavelet_levels):
            x = self.dwt_list[level](x)
            condition = self.dwt_list[level](condition)
        x = x.flatten(2).transpose(1, 2)
        condition = condition.flatten(2).transpose(1, 2)
        ...
        for block in self.blocks:
            x = block(x, c_t)    
            # x = checkpoint(block, x, c_t, use_reentrant=False)

        # TODO后续forward逻辑
        x = self.final_linear(self.final_norm(x))
        x, _ = x.chunk(2, dim=1)
        x = self.unpatchify_wavelet(x)
...
def DiT_S_8(**kwargs):
    return DiT(depth=12, hidden_size=384, patch_size=1, num_heads=6, input_size=256, in_channels=6, learn_sigma=False, **kwargs)
