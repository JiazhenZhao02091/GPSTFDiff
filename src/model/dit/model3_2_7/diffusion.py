...
class Diffusion(nn.Module):
    ...
    # ++++++++++++++++++++++++++++++++++++ Loss Functions ++++++++++++++++++++++++++++++++++++
    @staticmethod
    def charbonnier(x, y, eps: float = 1e-3):
        # return: same shape as x
        return torch.sqrt((x - y) ** 2 + eps ** 2)
    
    def grad_loss(self, pred, gt, eps: float = 1e-3):
        # pred/gt: (B,C,H,W)
        # 简单差分梯度（比 Sobel 更轻量）
        dx_p = pred[..., :, 1:] - pred[..., :, :-1]
        dy_p = pred[..., 1:, :] - pred[..., :-1, :]
        ...
        # per-sample loss: (B,)
        loss_dx = self.charbonnier(dx_p, dx_g, eps).mean(dim=(1, 2, 3))
        loss_dy = self.charbonnier(dy_p, dy_g, eps).mean(dim=(1, 2, 3))
        return loss_dx + loss_dy
    
    @staticmethod
    def sam_loss(pred, gt, eps: float = 1e-8):
        """
        pred/gt: (B,C,H,W)  (建议 pred/gt 非负更稳定；可先映射到[0,1])
        return: (B,) per-sample
        """
        B, C, H, W = pred.shape
        pred_v = pred.view(B, C, -1)  # (B,C,HW)
        gt_v   = gt.view(B, C, -1)

        dot = (pred_v * gt_v).sum(dim=1)  # (B,HW)
        pred_n = torch.linalg.norm(pred_v, dim=1)  # (B,HW)
        gt_n   = torch.linalg.norm(gt_v, dim=1)

        cos = dot / (pred_n * gt_n + eps)
    ...
    def p_losses(self, coarse_img_01, coarse_img_02, fine_img_01, fine_img_02, t, noise=None):
        ...
        # -------------------------
        # A) pixel loss (Charbonnier recommended)
        # -------------------------
        pix_per_sample = self.charbonnier(model_out, x0_t, eps=1e-3).mean(dim=(1, 2, 3))

        # -------------------------
        # B) gradient loss (edge / texture)
        # -------------------------
        grad_per_sample = self.grad_loss(model_out, x0_t, eps=1e-3)

        # -------------------------
        # C) SAM loss (spectral consistency)
        #    建议 SAM 在非负域更稳定；如果你的输入/输出是[-1,1]，映射到[0,1]
        # -------------------------
        x_hat_01 = (model_out + 1.0) * 0.5
        x_gt_01  = (x0_t      + 1.0) * 0.5
        sam_per_sample = self.sam_loss(x_hat_01, x_gt_01, eps=1e-8)
        # -------------------------
        # D) combine + p2 weight
        # -------------------------
        lambda_pix  = 1.0
        ...
        total_per_sample = (
            lambda_pix * pix_per_sample
            + lambda_grad * grad_per_sample
            + lambda_sam * sam_per_sample
        )
        total_per_sample = total_per_sample * extract(self.p2_loss_weight, t, total_per_sample.shape)
