...
class Diffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.fullres_predict_residual = True  # 只让全分辨率阶段预测 residual
        self.residual_scale = 1.0             # 如果你后面想把 residual 缩放到[-1,1]，这里可设为2.0等
    ...
    def ddim_sample(
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            clip_now = clip_denoised
            if self.mode == "x1+x2+x3":   # 且你这一阶段是 residual 模型
                clip_now = False
            ...
            if i == self.sample_T2:
                ...
                noisy_fine_img_02 = x_start_up * alpha_next.sqrt() + noise * (1 - alpha_next).sqrt() * scale
                continue
            ...
            if i == self.sample_T1:
                ...
                # 上采样上一阶段的 x_start（图像域）
                x_img = F.interpolate(x_start, scale_factor=2, mode='bilinear', align_corners=False)

                # residual 估计
                r_est = x_img - fine_img_01

                # 构造与你训练一致的 x0_t（残差金字塔 + alpha）
                x1, x2, x3 = self.laplacian_pyramid(r_est, levels=3)
                t_next = torch.full((batch,), time_next, device=device, dtype=torch.long)
                x0_t = self.get_x0_t(x1, x2, x3, t_next)

                # ✅ 沿用噪声：把上一阶段的 pred_noise 上采样当作 epsilon
                eps = F.interpolate(pred_noise, scale_factor=2, mode='bilinear', align_corners=False)

                # 用同一个 eps 在 time_next 构造 residual 的 x_{t_next}
                noisy_fine_img_02 = x0_t * alpha_next.sqrt() + eps * (1 - alpha_next).sqrt()
                continue
        ...
        out = noisy_fine_img_02
        if getattr(self, "fullres_predict_residual", True) and self.mode == "x1+x2+x3":
            out = out * getattr(self, "residual_scale", 1.0) + fine_img_01
        return out
