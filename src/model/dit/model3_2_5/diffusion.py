...
class Diffusion(nn.Module):
    ...
    def p_losses(self, coarse_img_01, coarse_img_02, fine_img_01, fine_img_02, t, noise=None):
        ...
        target_residual = fine_img_02 - fine_img_01
        x1, x2, x3 = self.laplacian_pyramid(target_residual, levels=3)
    ...
    def sample(self, coarse_img_01, coarse_img_02, fine_img_01):
        ...
        # 获取预测出的残差
        pred_residual = self.ddim_sample(
            coarse_img_01,
            coarse_img_02,
            fine_img_01,
            noisy_fine_img_02,
        )
         # 修改：将残差加回 f1 以恢复图像 (注意根据 mode 调整 f1 分辨率)
        if self.mode == "x1+x2+x3":
            return pred_residual + fine_img_01
        elif self.mode == "x2+x3":
            return pred_residual + F.interpolate(fine_img_01, scale_factor=0.5, mode='bilinear', align_corners=False)
        else:
            return pred_residual + F.interpolate(fine_img_01, scale_factor=0.25, mode='bilinear', align_corners=False)