...
class Diffusion(nn.Module):
    ...
    def p_losses(self, coarse_img_01, coarse_img_02, fine_img_01, fine_img_02, t, noise=None):
        ...
        model_out, m = self.model(
            noisy_fine_img_02, t, condition, return_mask=True
        )
        target = x0_t # TODO:x0_t or fine_img_02?

                # ===== change soft label y (B,1,H,W) from GT =====
        # [-1,1] 下 |f2-f1| 均值约在 [0,2]
        tau = 0.30
        alpha = 2.0
        lambda_mask = 0.1

        f1_c = condition[:, 12:18]              # (B,6,Hc,Wc)  已经对齐到 m 的分辨率

        Hc, Wc = f1_c.shape[-2], f1_c.shape[-1]
        f2_c = F.interpolate(fine_img_02, size=(Hc, Wc), mode='bilinear', align_corners=False)

        d = (f2_c - f1_c).abs().mean(dim=1, keepdim=True)   # (B,1,Hc,Wc)
        y = (d / tau).clamp(0, 1)

        # ===== main loss: change-weighted L1 on target =====
        loss_map = self.loss_fn(model_out, target, reduction='none')       # (B,C,H,W)
        loss_pix = loss_map.mean(dim=1, keepdim=True)                      # (B,1,H,W)
        w = 1.0 + alpha * y                                                # (B,1,H,W)
        loss_pix = loss_pix * w

        loss = reduce(loss_pix, 'b 1 h w -> b', 'mean')                    # (B,)
        loss = loss * extract(self.p2_loss_weight, t, loss.shape)          # (B,)
        loss_main = loss.mean()

        # ===== mask auxiliary =====
        loss_mask = (m - y).abs().mean()

        return loss_main + lambda_mask * loss_mask
