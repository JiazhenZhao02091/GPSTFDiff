...
class Pinwheel_shapedConv_Light(nn.Module):  
    def __init__(self, c1, c2, k, s, g):
        super().__init__()
        # 确保分支通道分配正确，处理不能整除的情况
        c_branch = c2 // 4
        p = [(k, 0, 1, 0), (0, k, 0, 1), (0, 1, k, 0), (1, 0, 0, k)]
        self.pad = [nn.ZeroPad2d(padding=(p[i])) for i in range(4)]
        
        # 保持深度卷积特性 g=g//4
        self.cw = Conv(c1, c_branch, (1, k), s=s, p=0, g=g // 4 if g > 1 else 1)
        self.ch = Conv(c1, c_branch, (k, 1), s=s, p=0, g=g // 4 if g > 1 else 1)
        
        # 最后的 cat 分支可能需要处理 c_branch*4 != c2 的情况
        self.cat = Conv(c_branch * 4, c2, 2, s=1, p=0, g=g)
...
class FeedForwardControl(nn.Module):
    ...
    def __init__(
        ...
        self.control_conv = zero_module(
            Pinwheel_shapedConv_Light(inner_dim, inner_dim, k=3, s=1, g=inner_dim)
        ) 
