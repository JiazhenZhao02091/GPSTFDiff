...
class FSDFormer(nn.Module):
    """FSDFormer model"""
    def __init__(self, in_dim=64, down_scale=2, depths=(2, 2, 6, 2), agent_num=[9, 16, 64, 64], in_channels=6):
...
if __name__ == '__main__':
    model = FSDFormer(in_dim=64, down_scale=2, depths=(2, 2, 6, 2), agent_num=[9, 16, 64, 64], in_channels=6)
    # print(model)
    input_fine = torch.randn((1, 6, 256, 256))
    input_coarse1 = torch.randn((1, 6, 256, 256))
    input_coarse0 = torch.randn((1, 6, 256, 256))
    output = model(input_fine, input_coarse1, input_coarse0)
    print(output.shape)