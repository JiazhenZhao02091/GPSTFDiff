...
class BiGaussianDiffusion(nn.Module):
    def __init__(
        self,
        model,
        *,
    ...
    def p_losses(
        ...
        # Use masked loss if mask is provided
        if mask is not None:
            loss = self.masked_l1_loss(model_out, target, mask)
        else:
            loss = self.loss_fn(model_out, target, reduction='none')
            loss = reduce(loss, 'b ... -> b', 'mean') # Changed from b (...) -> b (...) to match diffusion.py reduction logic if needed
