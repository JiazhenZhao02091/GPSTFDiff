...
"""
    bi_MKIRA_net: 
    结合了 Bi-Directional 输入逻辑 (01, 02, 03) 和 MKIRA/FiLM 架构。
    forward参数为 [coarse_img_01, coarse_img_02, coarse_img_03, fine_img_01, noisy_fine_img_02, fine_img_03, t, x_self_cond]
"""
...
# --------------------------
# main model (Bi-Directional + FiLM + MKIRA)
# --------------------------
class BiPredNoiseNetMKIRA(nn.Module):
    def __init__(
        self,
        dim: int = 64,
        init_dim: int = None,
        ...
        include_fine_in_cond: bool = True, # include fine_01 and fine_03 in conditioning
        ...
        # noisy stream input: for predicting fine_02
        self.noisy_init_conv = nn.Conv2d(input_channels, init_dim, 3, padding=1)

        # conditional stream input construction:
        # We have: coarse_01, coarse_02, coarse_03, fine_01, fine_03
        # Logic: 
        # 1. Coarse context: cat(c1, c2, c3) -> 3 * channels
        # 2. Differences: (c2 - c1), (c3 - c2) -> 2 * channels (flow-like info)
        # 3. Fine context: cat(f1, f3) -> 2 * channels (if included)
        
        self.include_fine_in_cond = include_fine_in_cond
        
        cond_ch_count = 0
        cond_ch_count += 3 * channels # coarse (01, 02, 03)
        cond_ch_count += 2 * channels # diffs
        if include_fine_in_cond:
            cond_ch_count += 2 * channels # fine (01, 03)

        self.cond_init_conv = nn.Conv2d(cond_ch_count, init_dim, 3, padding=1)
        ...
        for up_idx, (dim_in, dim_out_) in enumerate(reversed(in_out)):
            ...
            # apply MKIRA only on selected up stages
    ...
    def forward(
        ...
        coarse_img_01,
        coarse_img_02,
        coarse_img_03, # Bi-directional new input
        fine_img_01,
        noisy_fine_img_02,
        fine_img_03,   # Bi-directional new input
        time,
        x_self_cond=None,
    ):
        # self-conditioning
        ...
        # ----------------------------------------------------
        # Bi-Directional Condition Logic
        # ----------------------------------------------------
        # Differences (Flow approximation)
        dc_prev = coarse_img_02 - coarse_img_01
        dc_next = coarse_img_03 - coarse_img_02

        # Base conditioning (Coarse + Diffs)
        cond_list = [
            coarse_img_01, 
            coarse_img_02, 
            coarse_img_03, 
        ...
        if self.include_fine_in_cond:
            # Add fine contexts from both ends
            cond_list.extend([fine_img_01, fine_img_03])
        
        cond_in = torch.cat(cond_list, dim=1)
        # ----------------------------------------------------
        ...
        for up_idx, ((up_res, upsample), up_film, refiner) in enumerate(
            ...
            # MKIRA refine at selected decoder stages
            x = refiner(x)
...
if __name__ == "__main__":
    net = BiPredNoiseNetMKIRA(
        dim=64, channels=6, out_dim=6, dim_mults=(1, 2, 4),
        use_mkira=True, mkira_up_indices=(1, 2), mkira_init_alpha=0.0
    )
    b, c, h, w = 1, 6, 256, 256
    c1 = torch.randn(b, c, h, w)
    c2 = torch.randn(b, c, h, w)
    c3 = torch.randn(b, c, h, w) # future coarse
    f1 = torch.randn(b, c, h, w)
    f3 = torch.randn(b, c, h, w) # future fine
    noisy_f2 = torch.randn(b, c, h, w)
    time = torch.randint(0, 1000, (1,))
    
    out = net(c1, c2, c3, f1, noisy_f2, f3, time)
    print(f"Input shape: {c1.shape}")
