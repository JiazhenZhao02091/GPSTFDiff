...
class Trainer:
    ...
    def val(self):
        ...
        for key, value in self.val_tracker.results.items():
            ...
            if key == "SSIM":
                if value > self.best_ssim:
                    self.best_ssim = value
                    self.save_checkpoint()
                    msg += f" -- saved best model with SSIM: {self.best_ssim:.4f}"
        self.txt_logger.info(msg)
    ...
    def save_checkpoint(self, is_best=False):
        ...
        # 保存常规 checkpoint
        filename = f'model_epoch_{self.current_epoch}.pth'
        torch.save(data, self.train_checkpoints_dir / filename)
