...
class Trainer:
    ...
    def init_dataloader(self, train_dataloader, val_dataloader):
        ...
        self.max_train_iter = len(self.train_dataloader)
        self.max_val_iter = len(self.val_dataloader)
    ...
    def train_epoch(self):
        ...
        for key, value in self.train_tracker.results.items():  # loss + metrics
            msg += f', {key}: {value:.4f}'
    ...
    def train_iter(self, iter_idx, model_input_list):
        ...
        self.optimizer.step()
        msg = f'epoch: [{self.current_epoch + 1} / {self.max_epoch}], iter: [{(self.current_train_step + 1) % self.max_train_iter} / {self.max_train_iter}], loss: {loss.item():.4e}'
    ...
    def val(self):
        ...
        if (
            ...
            self.txt_logger.info(
                f'early stop at epoch {self.current_epoch}: '
    ...
    def val_iter(
        ...
        msg = f'val epoch: {self.current_epoch}, iter: [{(iter_idx + 1) % self.max_val_iter}/{self.max_val_iter}]'
        with torch.no_grad():
