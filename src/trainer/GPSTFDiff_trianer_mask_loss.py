...
"""
    和 stfdiff_trainer.py 的最大区别就是使用了 mask loss
    以及通过字典参数传入训练相关的参数
"""
# Masked Loss
class Trainer:
    def __init__(
        ...
        self.init_tracker()
        self.restore_best_metric_state_from_log()
    ...
    def restore_best_metric_state_from_log(self):
        log_paths = [self.train_txt_logs_dir / 'log.log']
        log_paths.extend(sorted(self.train_root_dir_path.glob('*.log')))

        target_name = str(self.best_metric_name).lower()
        pattern = re.compile(
            rf'best\s+{re.escape(target_name)}\s+updated:\s+'
            rf'([-+0-9.eE]+)\s+\(epoch\s+(\d+)\)',
            re.IGNORECASE,
        )
        ...
        if restored_value is not None:
            self.txt_logger.info(
                f'restored best {self.best_metric_name}: {restored_value:.6f} '
                f'(epoch {restored_epoch}) from log'
            )

    def train(self):
        while self.current_epoch < self.max_epoch and not self.stop_training:
            self.train_epoch()
    ...
    def get_model_input(self, data_per_batch):
        ...
        model_input_list = [coarse_img_01, coarse_img_02, fine_img_01, fine_img_02, mask]
        return model_input_list
    ...
    def val(self):
        ...
        if (
            ...
            and self.best_metric_epoch >= 0
            and self.current_epoch - self.best_metric_epoch >= self.early_stop_patience
        ):
            self.stop_training = True
            self.txt_logger.info(
                f'early stopping at epoch {self.current_epoch}: '
                f'best {self.best_metric_name} was {self.best_metric_value:.6f} '
                f'at epoch {self.best_metric_epoch}, patience '
    ...
    def before_val_iter(self, data_per_batch):
        ...
        metrics_mask = data_per_batch['mask'].to(self.device)
        show_img_list = [
