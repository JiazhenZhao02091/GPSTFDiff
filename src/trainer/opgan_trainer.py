...
class Trainer:
    ...
    def train_warm_up_epoch(self):
        ...
        for iter_idx, data_per_batch in enumerate(self.train_dataloader):
            ...
            self.train_warm_up_iter(
                iter_idx, model_generator_input_list, loss_gt, loss_l_diff_gt, metric_gt
            )
    ...
    def before_train_warm_up_iter(self, data_per_batch):
        ...
        loss_l_diff_gt = data_per_batch['fine_img_02'].to('cuda') - data_per_batch[
            'fine_img_01'
        ].to('cuda')
        metric_gt = data_per_batch['fine_img_02'].to('cuda')
        return (model_generator_input_list, loss_gt, loss_l_diff_gt, metric_gt)
    ...
    def train_warm_up_iter(
        self, iter_idx, model_generator_input_list, loss_gt, loss_l_diff_gt, metric_gt
    ):
        self.optimizer_generator.zero_grad()
        lr_g = self.optimizer_generator.param_groups[0]['lr']
        phase_g_outputs, fine_temporal_change_img = self.model_generator(
            *model_generator_input_list
        )
        pixel_loss = F.mse_loss(phase_g_outputs, loss_gt)
        change_loss = w_change * F.mse_loss(fine_temporal_change_img, loss_l_diff_gt)
        phase_g_loss = pixel_loss + change_loss
        phase_g_loss.backward()
        self.optimizer_generator.step()
        msg = f'warm up epoch: {self.current_epoch}, lr_g: {lr_g:.3e}, iter: {iter_idx}, pixel_loss: {pixel_loss.item():.4e}, change_loss: {change_loss.item():.4e}, loss: {phase_g_loss.item():.4e}'

        self.train_tracker.update('pixel_loss', pixel_loss.item())
        self.train_tracker.update('change_loss', change_loss.item())
        self.train_tracker.update('phase_g_loss', phase_g_loss.item())

        self.backend_logger.add_scalar(
            'train_runing/pixel_loss', pixel_loss.item(), self.current_train_step
        )
        self.backend_logger.add_scalar(
            'train_runing/change_loss', change_loss.item(), self.current_train_step
        )
        self.backend_logger.add_scalar(
            'train_runing/loss', phase_g_loss.item(), self.current_train_step
        )
    ...
    def train_iter(
        ...
        with torch.no_grad():
            phase_d_fake_output_g, _ = self.model_generator(*model_generator_input_list)
        ...
        self.optimizer_generator.zero_grad()
        phase_g_fake_output_g, fine_temporal_change_img = self.model_generator(
            *model_generator_input_list
        )
        phase_g_fake_output_d = self.model_dicriminator(phase_g_fake_output_g)
        phase_g_fake_loss = -torch.mean(phase_g_fake_output_d) * 1e-2

        phase_g_gan_loss = phase_g_fake_loss        # 对抗损失
        pixel_loss = F.mse_loss(phase_g_fake_output_g, loss_gt) # MSE loss
        change_loss = w_change * F.mse_loss(fine_temporal_change_img, loss_l_diff_gt)  # 变化损失
        phase_g_loss = phase_g_gan_loss + pixel_loss + change_loss
        phase_g_loss.backward()
        self.optimizer_generator.step()

        msg += f', phase_g_fake_loss: {phase_g_fake_loss.item():.4e}, pixel_loss: {pixel_loss.item():.4e}, change_loss: {change_loss.item():.4e}, phase_g_loss: {phase_g_loss.item():.4e}'
        ...
        self.backend_logger.add_scalar(
            f'train_running/change_loss', change_loss.item(), self.current_train_step
        )
        self.backend_logger.add_scalar(
    ...
    def val_iter(
        ...
        show_dir_prefix = (
            f'{dataset_name}/{self.current_epoch}/show_img/fine_temporal_change_img'
        )
        show_dir_path = self.train_imgs_dir / show_dir_prefix
        show_name = key.split('-')[-1]
        show_name = (
            show_name[:8] + '_show_fine_temporal_change_img_' + show_name[9:] + '.png'
        )
        if 'STIL' in save_dir_prefix:
            show_name = key + '.png'
        self.img_diff_show(
            show_img_diff,
            fine_temporal_change_img,
            show_dir_path,
            show_name,
        )
