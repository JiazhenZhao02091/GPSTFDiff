...
class Trainer:
    ...
    def get_loss_gt(self, data_per_batch):
        fine_img_02 = data_per_batch['fine_img_02'].to(self.device)
        x1, x2, x3, x4 = self.model.laplacian_pyramid(fine_img_02, levels=4)
        ...
        if self.mode == "x4":
            ...
            x4_up = F.interpolate(x4, scale_factor=4, mode='bilinear', align_corners=False)
            x3_up = F.interpolate(x3, scale_factor=2, mode='bilinear', align_corners=False)
            return x2 + x3_up + x4_up
        else:  # "x1+x2+x3+x4"
            x4_up = F.interpolate(x4, scale_factor=8, mode='bilinear', align_corners=False)
