...
class Trainer:
    ...
    def get_loss_gt(self, data_per_batch):
        "change scale  (64 64)"
        fine_img_02 = self.downsample(self.downsample(data_per_batch['fine_img_02']))
        return fine_img_02.to(self.device)
    ...
    def val_iter(
        ...
        print(f"loss_gt max: {loss_gt.max()}, loss_gt min: {loss_gt.min()}")
        print(f"metrics_gt max: {metrics_gt.max()}, metrics_gt min: {metrics_gt.min()}")
