...
class Trainer:
    def __init__(
        ...
        self.model_py_path = Path(inspect.getsourcefile(self.model.__class__))
        shutil.copy(self.model_py_path, self.train_configs_dir)
    ...
    def get_loss_gt(self, data_per_batch):
        fine_img_02 = data_per_batch['fine_img_02'].to(self.device)
        fine_img_02 = F.interpolate(fine_img_02, scale_factor=0.5, mode='bilinear', align_corners=False)
        return fine_img_02
    ...
    def val_iter(
        ...
        with torch.no_grad():
            ...
            print(f"outpus.shape is {outputs.shape}, loss_gt.shape is {loss_gt.shape}")
            assert outputs.shape == loss_gt.shape, "outputs and loss_gt shape must be same"
