...
# 假设 FusionLogger, Tracker, 以及 metric_list 都已经正确定义并可用
...
class Trainer:
    def __init__(
        ...
        self.accelerator = Accelerator(
            ...
            split_batches=False,  # 如果你的 dataloader batch 很大需要拆分
    ...
    def init_model_and_optimizer_and_scheduler(self):
        ...
        # optimizer: expect optimizer_cls to be a callable factory like:
        # optimizer_cls = lambda params: torch.optim.AdamW(params, lr=1e-4)
        assert self._orig_optimizer is not None, "optimizer must be provided"

        # Create optimizer with model.parameters() (model is still unprepared)
        self.optimizer = self._orig_optimizer

        # scheduler: if provided, create later; but it's okay to create now using optimizer
        self.scheduler = (
            self._orig_scheduler(self.optimizer) if self._orig_scheduler is not None else None
        )

        # Now prepare model, optimizer, dataloaders (in one call)
    ...
    # -------------------------
    # Data helpers (do NOT .to(device) here)
    # -------------------------
    def get_model_input(self, data_per_batch):
        # accelerator already moved tensors to device, so just take them
    ...
    def get_loss_gt(self, data_per_batch):
        # If you need values on device, they are already on device
        fine_img_02 = data_per_batch["fine_img_02"]
    ...
    def train(self):
        while self.current_epoch < self.max_epoch:
            if self.scheduler is not None:
                # if you want step per epoch; otherwise move scheduler step into loop
                try:
                    self.scheduler.step()
                except Exception:
                    pass
    ...
    def train_epoch(self):
        ...
        # EMA update after epoch if you want per-epoch update; keep per-step updates in train_iter if update_every=1
    ...
    def train_iter(self, iter_idx, model_input_list):
        ...
        with self.accelerator.accumulate(self.model):
            ...
            # EMA update: ema.step or ema.update depending on library
            # ema_pytorch's EMA object updates via ema.update() (it pulls parameters from original model)
            # We should use the unwrapped model for EMA updates; ema was created with the unwrapped model.
            # if self.accelerator.sync_gradients:
            #     self.ema.update()
        ...
        with torch.no_grad():
            ...
            for metric in self.metric_list:
                try:
                    ...
                    # if metric expects CPU numpy, move to cpu
                ...
                if self.accelerator.is_main_process and self.backend_logger is not None:
                    self.backend_logger.add_scalar(f"val_runinng/{metric_name}", metric_value.item(), self.current_val_step)
