...
"""
    基于 LapSTFDiff_trianer_mask_loss.py 修改
    支持双向输入 (bi-directional) + Masked Loss
"""
# Masked Loss + Bi-Directional
class Trainer:
    def __init__(
        self,
        congfig_path,
        ...
        self.init_train_params(train_params)
    ...
    def init_train_params(self, train_params):
        ...
        # use provided train_params to override default settings
    ...
    def get_model_input(self, data_per_batch):
        ...
        coarse_img_03 = data_per_batch['coarse_img_03'].to(self.device) # Added 03
        fine_img_01 = data_per_batch['fine_img_01'].to(self.device)
        fine_img_02 = data_per_batch['fine_img_02'].to(self.device)
        fine_img_03 = data_per_batch['fine_img_03'].to(self.device)     # Added 03
        mask = data_per_batch['mask'].to(self.device)
        
        # Consistent with bi_diffusion.py forward method
        model_input_list = [
            coarse_img_01,
            coarse_img_02,
            coarse_img_03,
    ...
    def val(self):
        ...
        # --- add: save best metric checkpoint ---
        target_name = str(self.best_metric_name).lower()
        metric_key = None
        for k in self.val_tracker.results.keys():
            if str(k).lower() == target_name:
                metric_key = k
                break
    
        if metric_key is not None:
            current_value = self.val_tracker.results[metric_key]
            is_better = (
            ...
            if is_better:
                ...
                self.txt_logger.info(
                    f'best {metric_key} updated: {self.best_metric_value:.6f} '
                    f'(epoch {self.best_metric_epoch}), saved: best_model_epoch_{self.current_epoch}.pth'
                )
        if self.scheduler is not None:
            val_rmse = self.val_tracker.results[metric_key]
            self.txt_logger.info(
                f'Scheduler step with val {metric_key}: {val_rmse}'
    ...
    def get_model_sampling_input(self, data_per_batch):
        ...
        # Note: bi_diffusion q_sample / sample method usually takes these 6 inputs (including mask if updated)
        # But crucially, 'noise' or 'target' is handled internally or omitted during sampling
        model_sampling_input_list = [
            coarse_img_01,
            coarse_img_02,
            coarse_img_03,
            fine_img_01,
            fine_img_03, # Note: check if fine_img_02 is needed for sampling shape reference in your specific implementation
        ]
        
        # Refined for bi_diffusion sample method signature:
        # If your bi_diffusion.sample() expects (c1, c2, c3, f1, f3, mask), remove fine_img_02
        # However, many implementations use the ground truth just to get shape (b,c,h,w)
        # Assuming sample method signature matches forward but without target (fine_02)
        
        # Let's adjust to match what bi_stfdiff_trainer does + mask:
        model_sampling_input_list = [
            coarse_img_01,
            coarse_img_02,
            coarse_img_03,
    ...
    def val_iter(
        ...
        with torch.no_grad():
            outputs = self.ema.ema_model.sample(*model_sampling_input_list)  # sample process
